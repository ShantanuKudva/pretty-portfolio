...
interface ProfileCardProps {
  ...
  onContactClick?: () => void;
  // Included props from manual call example
  showIcon?: boolean;
...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  useEffect(() => {
    ...
    const handleDeviceOrientation = (event: DeviceOrientationEvent) => {
      ...
      const y = clamp(shell.clientHeight / 2 + (beta - ANIMATION_CONFIG.DEVICE_BETA_OFFSET) * mobileTiltSensitivity, 0, shell.clientHeight);
    ...
    const initialX = shell.clientWidth - ANIMATION_CONFIG.INITIAL_X_OFFSET;
  ...
  }, [enableTilt, enableMobileTilt, tiltEngine, mobileTiltSensitivity]);

  const cardStyle = useMemo(
    () => ({
      '--icon': (showIcon && iconUrl) ? `url(${iconUrl})` : 'none',
      '--grain': grainUrl ? `url(${grainUrl})` : 'none',
      '--inner-gradient': customInnerGradient ?? innerGradient ?? DEFAULT_INNER_GRADIENT,
  ...
  const effectiveShowBehindGlow = behindGlowEnabled && showBehindGlow;

  return (
    <div ref={wrapRef} className={`${styles['pc-card-wrapper']} ${className}`.trim()} style={cardStyle}>
      {effectiveShowBehindGlow && <div className={styles['pc-behind']} />}
