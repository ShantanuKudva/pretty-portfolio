...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = props => {
  ...
  const { hoverIntensity, animated, onAnimationComplete, duration, fadeOutOnScroll } = config as any;
  const [scrollOpacity, setScrollOpacity] = useState(1);

  useEffect(() => {
    if (!fadeOutOnScroll) return;
    const handleScroll = () => {
      ...
      const fadeStart = maxScroll - 500; // Start fading after the contact card
      const fadeEnd = maxScroll - 20;   // Disappear right before the footer
      ...
      if (scrollY < fadeStart) {
        ...
        // Use a quadratic easing for a smoother "gradual" feel
        const p = (scrollY - fadeStart) / (fadeEnd - fadeStart);
        setScrollOpacity(1 - p * p);
      }
    };
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const baseStyle: CSSProperties = {
      ...
      opacity: (isVisible ? 1 : 0) * (typeof scrollOpacity !== 'undefined' ? scrollOpacity : 1) * (config.opacity ?? 1),
